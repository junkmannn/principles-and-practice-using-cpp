...
public:
	...
	// deal with color and draw_lines
	void draw() const;

protected:
	Shape() : lcolor(fl_color()), ls(0), fcolor(Color::invisible) { }
...
struct Line : Shape {
	...
	Line(Point p1, Point p2, Color color, Line_style ls) : Line(p1, p2) {
		set_color(color);
		set_style(ls);
	}
};
...
struct Rectangle : Shape {
	...
	{
		if (h <= 0 || w <= 0) std::runtime_error("Bad rectangle: first point is not top left");
		add(x);
	}
...
struct Lines : Shape {	// indepentdent lines
	Lines() {}
	Lines(std::initializer_list<Point> lst) : Shape(lst) { if (lst.size() % 2) std::runtime_error("odd number of points for Lines"); }
	void draw_lines() const;
...
struct Circle : Shape {
	...
	Circle(Point p, int r, Color color, Color fcolor, Line_style ls) : Circle(p, r) {
		set_color(color);
		set_fill_color(fcolor);
		set_style(ls);
	}

	void draw_lines() const;

	void set_center(Point p) { set_point(0, Point(p.x-r, p.y-r)); }
