...
void Polygon::add(Point p)
{
	...
	if (1<np) {	// check that thenew line isn't parallel to the previous one
		...
		if (parallel)
			std::runtime_error("two polygon points lie in a straight line");
	}

	for (int i = 1; i<np-1; ++i) {	// check that new segment doesn't interset and old point
		Point ignore(0,0);
		if (line_segment_intersect(point(np-1),p,point(i-1),point(i),ignore))
			std::runtime_error("intersect in polygon");
	}
...
{
	if (number_of_points() < 3) std::runtime_error("less than 3 points in a Polygon");
...
{
	if (r2 - r1 <= 0) std::runtime_error("bad graphing range");
	if (count <= 0) std::runtime_error("non-positive graphing count");
...
Axis::Axis(Orientation d, Point xy, int length, int n, std::string lab)
	:label(Point(0,0),lab)
{
	if (length<0) std::runtime_error("bad axis length");
	switch (d){
	case Axis::x:
	...
	case Axis::z:
		std::runtime_error("z axis not implemented");
	}
}
