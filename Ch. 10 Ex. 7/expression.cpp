...
double Expression::expression(char exit) {
	...
	while (true) {
		...
		default:
			if (exit == 0) {
				if (token.get_kind() != Token::END) throw Syntax_error("unexpected token");
			}
			else {
				if (token.get_kind() == Token::END) char_expected(exit); // throws Syntax_error
				else if (token.get_syntax() != exit) throw Syntax_error("unexpected token");
			}
...
double Expression::term() {
	...
	while (true) {
		...
		{
			double x = primary();
			if (x == 0) throw Syntax_error("divide by zero");
			term /= x;
			break;
		}
		default:
...
double Expression::primary() {
	...
	case Token::NAME:
	{
		...
		if (!nvt.get(name, name_value)) throw Syntax_error(name + " undefined");
...
double Expression::solve_parentheses(char close_parenthesis) {
	double x = expression(close_parenthesis);
	/*
	if (ts.get().get_syntax() != close_parenthesis) {
		string msg = "x expected";
		msg[0] = close_parenthesis;
...
double Expression::solve_sqrt() {
	if (ts.get().get_syntax() != '(') throw Syntax_error("( expected");

	double x = expression(')');
	if (x < 0) throw Syntax_error("square root of a negative number");
